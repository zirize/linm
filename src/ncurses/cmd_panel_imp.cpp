...
///	@param	bPause	when quit on command line, pause to determine.
/// @param	bFork	Is Fork?
///	@return	return command.
int CmdPanelImp::Run( const std::string &cmd, bool bPause, bool bBackground)
{
	// when input 'quit' on command line, linm is quit.
...
///	@brief	all command message management function.
///	@param	p	Input the command to be executed.
///	@return		The command will return the results.
int CmdPanelImp::ParseAndRun(const string &p, bool Pause)
{
	...
	// Change directory(cd) is implemented.
	if (p.substr(0, 2) == "cd")
	{
		string sStr;
		if (p == "cd" || p == "cd ")
	...
	// for the files copy and viewing.
	{
...
void CmdPanelImp::EditorChoice(bool bReadOnly, const string& sEditorCmd, File* pFile)
{
	...
	if (!bExtEditor)
	{
		...
		// title is view the original source.
		if ( pFile )
		{
...
void CmdPanelImp::Quit()
{
	...
	g_tMainFrame.SaveConfig(); // at this point, Save config.
	g_tMainFrame.Exit();
}
...
void	CmdPanelImp::CopyPaste()
{
	...
	else
	{
		...
		// The archive reader is if select directory, uncompress with the under directory contents.
		// so not searching the sub-directory. it is long time.
			...
			{	
				...
				if (pCurReader->Paste(tSelectionMove))
					if ((pReader = _pPanel->GetReader("file://")) != NULL)
					{
						...
						pCurReader->Remove(_tMainSelection, false); // remote the original sources.
					}
			}
		}
...
void    CmdPanelImp::SortChange()
{
	...
	switch(nSort)
	{
		case SORT_NONE: SortNone();    break;
		case SORT_NAME: SortName();    break;
		case SORT_EXT: SortExt();    break;
		case SORT_SIZE: SortSize();    break;
		case SORT_TIME: SortTime(); break;
		case SORT_COLOR: SortColor(); break;
	}
}
...
void 	CmdPanelImp::NewFile()
{
	...
	// title is view the original file.
...
/// @brief    Compress the selected files with matched the each type.
...
void	CmdPanelImp::Extract()
{
	...
	switch(nZipType)
	{
		...
		case RPM:
		{
			...
			switch(nSelect )
			{
				case 0:    // execute the root account.
					sCommand = "rpm -Uvh " + sFilename + " %R %W";
					break;
				case 1: // execute the root account.
...
bool GetPromptToPath( const string& sPrompt, string& sRtPath )
{
	// la9527@la9527-desktop:~/mls/trunk/mls2/src/ncurses$
	char	cHostName[100];
	...
	// la9527@la9527-desktop:~$
	string sPrompt2;
	bool bFind = false;
	int nFirst = -1;
	for ( int n = 0; n < sPrompt.size(); n++ )
	{
		if ( sPrompt[n] == ':' )
			nFirst = n;		
		if ( nFirst != -1 && (sPrompt[n] == '$' ||  sPrompt[n] == '#') )
		{
			sPrompt2 = sPrompt.substr( nFirst+1, n-nFirst-1 );
			bFind = true;
			break;
		}
	}	
...
void	CmdPanelImp::ConsoleMode()
{
	...
	// when change the terminal size of subshell, auto change.
	...
	if ( !sPrompt.empty() )
	{
		// When Remote(sftp, ftp) connection, not use the prompt and command line.
		char	cHostName[100];
		memset(&cHostName, 0, sizeof(cHostName));
		if ( gethostname((char*)&cHostName, sizeof(cHostName)) != -1 )
		{
	...
	// unblocking the screen size of subshell.
	Signal_ResizeUnblocking();
	CursesInit( g_tCfg.GetBool("Default", "Transparency", false ) );

	if ( !sPrompt.empty() )
	{
		if ( GetPromptToPath( sPrompt, sCurDir ) )
		{
			if ( sCurDir != _pPanel->GetReader()->GetPath() )
			{
